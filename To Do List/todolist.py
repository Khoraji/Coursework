...
class MyGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Huskelapp Alpha")
    ...
    def remove_todo(self):
        if len(self.listView.selectedIndexes()) != 0:
            ...
            dialog = QMessageBox()
            dialog.setText(f"Har du fullfort '{selected.data()}'?")
    ...
    def open_file(self):
       ...
       filename, _ = QFileDialog.getOpenFileName(self, "Open File", "", "todo (*.txt)", options=options)
       if filename != "":
    ...
    def save_file(self):
        ...
        options = QFileDialog.Options()
        filename, _ = QFileDialog.getSaveFileName(self, "Save File", "", "todo (*.txt)", options=options)
        if filename != "":
